...
public class productsDao {
    ...
    public int addProducts(products p){
       String sql="insert into products(P_ID,P_NAME,price,IMGPATH,CAT_ID) values("+p.getP_id()+",'"+p.getP_name()+"',"+p.getCost()+",'"+p.getImgpath()+"',"+p.getCat().getCat_id()+")";
...
return list.get(0);
        ...
        return  template.query("select * from product where cat_id=?",new Object[]{id},new RowMapper<products>(){    
            @Override
            public products mapRow(ResultSet rs, int row) throws SQLException {    
                products e=new products();
    ...
    //            e.setCat_name(rs.getString(2));
       ...
       String sql="update products Set P_NAME='"+p.getP_name()+"',cost="+p.getCost()+",IMAGE_PATH='"+p.getImgpath()+"',CAT_ID="+p.getCat().getCat_id()+" where p_id="+p.getP_id()+"";
       return template.update(sql);
    }
